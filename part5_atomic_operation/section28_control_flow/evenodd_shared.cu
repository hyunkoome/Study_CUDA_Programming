...
__global__ void kernelEvenOdd( float* dst, const float* src, unsigned num, unsigned half ) {
	...
	// read from global memory => shared memory
	s_src[tx] = src[gx];
	...
	if (tx % 2 == 0) {
		...
		s_dst[tx / 2 + BLOCK_SIZE / 2] = s_src[tx]; // BLOCK_SIZE / 2 = half
	}
	__syncthreads();

	// write to the global memory from shared memory
	register unsigned shift = blockIdx.x * blockDim.x / 2; // blockIdx.x * blockDim.x: 전체 개수
	if (tx < BLOCK_SIZE / 2) { // half = BLOCK_SIZE / 2
