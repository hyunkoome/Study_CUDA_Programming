...
void even_odd_sort( uint32_t* data, unsigned num ) {
	for (register unsigned i = 0; i < num / 2; ++i) {
		// even phase
		for (register unsigned j = 0; j < num - 1; j += 2) {
			comparator<dir>( data[j], data[j + 1] );
		}
		// odd phase
		for (register unsigned j = 1; j < num - 1; j += 2) {
			comparator<dir>( data[j], data[j + 1] );
		}
	}
}
...
int main(const int argc, const char* argv[]) {
	...
	if (direction == INCREASING) {
		for (unsigned i = 0; i < num_units; ++i) {
			even_odd_sort<INCREASING>( dst + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE) );
		}
	} else {
		for (unsigned i = 0; i < num_units; ++i) {
			even_odd_sort<DECREASING>( dst + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE) );
		}
	}
	ELAPSED_TIME_END(0);
