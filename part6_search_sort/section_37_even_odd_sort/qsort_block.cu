...
int main(const int argc, const char* argv[]) {
	...
	if (direction == INCREASING) {
		for (unsigned i = 0; i < num_units; ++i) {
			qsort( dst + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE), sizeof(uint32_t), compLess );
		}
	} else {
		for (unsigned i = 0; i < num_units; ++i) {
			qsort( dst + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE), sizeof(uint32_t), compGreater );
		}
	}
	ELAPSED_TIME_END(0);
	// show the result
	...
	for (unsigned i = 0; i < num_units; ++i) {
		printVec( "src", src + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE) );
	}
