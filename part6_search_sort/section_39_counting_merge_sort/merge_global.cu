...
const unsigned TOTAL_NUM = 16 * (2 * 1024); // max total num data
...
void deviceMergeSort( unsigned TOTAL_NUM, uint32_t* dev_vecA, uint32_t* dev_vecB, int dir ) {
	...
	for (unsigned segment_size = 2 * BLOCK_SIZE; segment_size < TOTAL_NUM; segment_size <<= 1) {
		...
		CUDA_CHECK_ERROR();
#if 1
		cudaMemcpy( rankA, dev_rankA, (segment_size / SAMPLE_STRIDE) * sizeof(unsigned), cudaMemcpyDeviceToHost );
		printVec("rankA", rankA, segment_size / SAMPLE_STRIDE);
		cudaMemcpy( rankB, dev_rankB, (segment_size / SAMPLE_STRIDE) * sizeof(unsigned), cudaMemcpyDeviceToHost );
		printVec("rankB", rankB, segment_size / SAMPLE_STRIDE);
#endif
...
#if 1
		cudaMemcpy( limitA, dev_limitA, (segment_size / SAMPLE_STRIDE) * sizeof(unsigned), cudaMemcpyDeviceToHost );
		printVec("limitA", limitA, segment_size / SAMPLE_STRIDE);
		cudaMemcpy( limitB, dev_limitB, (segment_size / SAMPLE_STRIDE) * sizeof(unsigned), cudaMemcpyDeviceToHost );
		printVec("limitB", limitB, segment_size / SAMPLE_STRIDE);
#endif
